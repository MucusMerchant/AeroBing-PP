...
from lib.imu_calib.imu_model import sensor_error_model, misalignment
from lib.imu_calib.quaternion import Quaternion
...
# separate function to get all the component matrices/vectors
# this is to avoid recalculating for each new measurement
def acceleration_equation_components(sensor_errors):
    M, b = sensor_error_model(sensor_errors[0:9])
    C = np.linalg.inv(M)

    return C, b

def gyroscope_equation_components(sensor_errors, epsilon):
    M, b = sensor_error_model(sensor_errors[0:9])
    C = np.linalg.inv(M)
    Rm = np.linalg.inv(misalignment(epsilon))
...
# function to process a SINGLE acceleration measurement
def calibrate_accelerometer_single(measurement, C, b):
    measurement = C @ (measurement - b)
    return measurement

def calibrate_gyroscope_single(measurement, C, b, Rm):
    measurement = C @ Rm @ measurement - C @ b
    return measurement
